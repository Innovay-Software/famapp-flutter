...
class ImLocalDatasourceRealm {
  late Database imDatabase;
  String _databaseFilePath = '';

  ImLocalDatasourceRealm._();
  static ImLocalDatasourceRealm? _instance;
  static ImLocalDatasourceRealm get instance {
    if (_instance == null) {
  ...
  static Future<ImLocalDatasourceRealm> init() async {
    if (_instance != null) {
      return _instance!;
    }

    _instance = ImLocalDatasourceRealm._();
    var postFix = InnoGlobalData.useRegionRemote ? 'cn' : 'ca';
    postFix = 'ca';
  ...
  Future<int> saveImMessages({
    ...
    List<LivechatMessage> imMessages = const [],
  ...
  Future<int> _saveImMessage({LivechatMessage? imMessage, dynamic imMessageData, int lastSyncedMessageId = -1}) async {
    ...
    //
    // final int imGroupId =
    //     imMessage != null ? imMessage.groupId : (int.tryParse('${imMessageData['im_group_id']}') ?? 0);
    // final String userUUID = imMessage != null ? imMessage.owner : ('${imMessageData['owner']}');
    // final String messageType = imMessage != null ? imMessage.type : '${imMessageData['type']}';
    // final String messageBody = imMessage != null ? imMessage.content : '${imMessageData['body']}';
